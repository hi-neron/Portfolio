...
function randomNum (max = 1, min = 0) {
...
function randomPhrase () {
  ...
  return RANDOM_PHRASES[phraseIndex]
}

module.exports = function (message) {
  let container = document.createElement('div')
  container.setAttribute('class', 'loader-icon-wrapper')
...
`
  ...
  let messageGenerated = message
  if (message === 'random') {
    messageGenerated = randomPhrase()
  }

  let messageContainer = yo`
  <div className="loader-message">
    ${messageGenerated}
  </div>
  `
  let tBLine = new TimelineMax({
    repeat: -1,
    yoyo: true,
  })
  ...
  container.appendChild(umbrellaLoader)
